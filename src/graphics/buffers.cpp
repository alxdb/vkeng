...
#include <vkeng/graphics/buffers.hpp>
...
Buffer::Buffer(const Device &device, size_t size, vk::BufferUsageFlags usage, vk::MemoryPropertyFlags memoryProperties)
    : size(size),
      buffer(device.handle.createBuffer({
          .size = size,
          .usage = usage,
          .sharingMode = vk::SharingMode::eExclusive,
      })),
      memory(allocateMemory(device.handle, device.details.physicalDevice, buffer, memoryProperties)) {
